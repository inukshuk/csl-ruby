...
module CSL
	describe 'Treelike' do
	  ...
	  before(:all) do 
			class Tree; include Treelike; end
		end
	  ...
	  describe '#children' do
	    ...
	    context 'when it has children' do
	      before(:each) { node.add_children([Tree.new, Tree.new, Tree.new]) }
	  ...
	  describe '#add_children' do
	    it 'adds a single node to the children array' do
	      node.add_children(Tree.new).children.should have(1).item
	    end
    
	    it 'adds a list of nodes to the children array' do
	      node.add_children(Tree.new, Tree.new).children.should have(2).item
	    end
	    ...
	    it 'sets self as parent of the added nodes' do
	      node.add_children([Tree.new, Tree.new]).children.map(&:parent).uniq.should == [node]
	    end
	  ...
	  describe '#ancestors' do
	    ...
	    context 'when it has ancestors' do
	      ...
	      let(:node1) { node.add_children(Tree.new).children[0] }
	      let(:node2) { node1.add_children(Tree.new).children[0] }
	      let(:node3) { node2.add_children(Tree.new).children[0] }
	  ...
	  describe 'named child accessors' do
	    ...
	    before(:all) { Tree.instance_eval { attr_children :tree } }
    
	    context 'when it has no children' do
	      it 'returns an empty list by default' do
	        Tree.new.tree.should be_empty
	      end
	    end
    
	    context 'when it has children' do
	      it 'returns a list of a single child with the matching name' do
	        Tree.new.add_children(Tree.new).tree.should have(1).item
	      end

	      it 'returns a list of all children with the matching name' do
	        Tree.new.add_children(Tree.new, Tree.new).tree.should have(2).items
	      end
	    end
	  end
